...
class EPSCalculator:
    ...
    def calculate_eps(self, module_name: str, module_uniquekey: int = None,
        ...
        for submodule_name in submodules:
            ...
            # Get uniquekey for this submodule
            ...
            sum_of_submodule_uniquekeys += uniquekey

        # If no sub-modules present, use default as 1
        if not submodules or sum_of_submodule_uniquekeys == 0:
            sum_of_submodule_uniquekeys = 1

        # Calculate final EPS using the correct formula:
        # EPS = Module level unique key × (sum of sub_module unique keys) / period
        ...
        eps = (module_level_keys * sum_of_submodule_uniquekeys) / period_seconds
        ...
        return {
            ...
            "sum_of_submodule_uniquekeys": sum_of_submodule_uniquekeys,
            "eps": eps,
            "calculation": {
                "formula": "EPS = Module level unique key × (sum of sub_module unique keys) / period",
    ...
    def suggest_uniquekey_for_target_eps(self, module_name: str, target_eps: float,
        ...
        # Calculate current sum of submodule unique keys
        sum_of_submodule_uniquekeys = 0
        submodule_configs = {}
        ...
        # EPS = Module level unique key × (sum of sub_module unique keys) / period
        # So: Module level unique key = (target_eps × period_seconds) / sum_of_submodule_uniquekeys

        required_module_uniquekey = (target_eps * period_seconds) / sum_of_submodule_uniquekeys

        # Round to nearest integer
        suggested_module_uniquekey = max(1, round(required_module_uniquekey))

        # Calculate expected EPS with suggested value
        expected_eps = (suggested_module_uniquekey * sum_of_submodule_uniquekeys) / period_seconds
